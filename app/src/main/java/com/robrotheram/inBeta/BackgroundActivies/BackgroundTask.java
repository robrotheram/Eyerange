...
public class BackgroundTask extends AsyncTask<String, Integer, Double> {
    ...
    private ListConstants.TYPE type;
    private HttpResponse httpResponse;

    public BackgroundTask(NetworkFeedback networkFeedbackresponse, DataStore dataStore,List<NameValuePair> nameValuePairs, ListConstants.TYPE type){
    ...
    protected void onPostExecute(Double result){
        ...
        try {
            ...
            dataStore.setRawJson(jObj);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        try {
            jObj = new JSONObject(json);
            networkFeedbackresponse.onPostComplete(dataStore, type);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        try {
    ...
    public void postData() {
        ...
        HttpClient ht = dataStore.getDefaultHttpClient();
        HttpPost httppost = new HttpPost("http://eyerange.co.uk/api/");

        try {
            httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs));
            httpResponse =  ht.execute(httppost);
